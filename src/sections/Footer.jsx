...
const companyHrefs = ['#portfolio', '#why-us', '#blog', '#pricing'];
...
export default function Footer() {
  ...
  const footerLinks = {
    ...
    [f.legal]: [
      { label: f.legalLinks[0], href: '#', onClick: () => setLegalModal('privacy') },
      { label: f.legalLinks[1], href: '#', onClick: () => setLegalModal('terms') },
      { label: f.legalLinks[2], href: '#' },
    ],
  };
  ...
  return (
    <>
      <footer className="relative border-t border-white/6 overflow-hidden" aria-label="Site footer">
        {/* Top glow line */}
        <div
          ...
          <div className="py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
            ...
            <div className="lg:col-span-2">
              ...
              >
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-cyan-400/10 border border-cyan-400/30 flex items-center justify-center p-1">
                ...
                <ul className="space-y-3" role="list">
                  {links.map(({ label, href, onClick }) => (
                    <li key={label}>
                      <a
                        href={href}
                        onClick={onClick ? (e) => { e.preventDefault(); onClick(); } : (e) => handleNavClick(e, href)}
                        className="text-sm text-slate-400 hover:text-cyan-400 transition-colors duration-200 flex items-center gap-1 group cursor-pointer"
                      >
                        {label}
